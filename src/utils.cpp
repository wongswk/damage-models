...
double dnorm(double x, double mu, double sig, bool logp = false){
...
double log_prior(vector<double> theta, int n_par){
	...
	for(int i = 0; i < n_par-2; i+=2){
		logp += dnorm(theta[i], 0, 20, true);	
	}
	logp += dnorm(theta[8], 0, 1, true); // stronger informative prior for sigma_0's mean (average 50% threshold)
